...
const CreatePackageModal = ({ isOpen, onClose, onPackageCreated }: CreatePackageModalProps) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!formData.title || !formData.subtitle || !formData.tourDetails || !formData.price || !formData.duration || !formData.location || !formData.capacity || !formData.packageType) {
      ...
      const packageData = {
        ...
        price: price,
        // Set default values for removed fields (required by model)
        place: (() => {
          // Convert location to place format (lowercase, replace spaces with hyphens)
          const locationToPlace = formData.location.toLowerCase().trim().replace(/\s+/g, '-');
          // Map common locations to valid place values
          ...
          return placeMap[locationToPlace] || (formData.packageType === 'international' ? 'dubai' : 'darjeeling');
        })(),
